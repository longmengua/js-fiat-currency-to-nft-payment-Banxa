...
export default async function banxaBuyNft(
  ...
  const json: BanxaNFTOrderI<any> = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
  ...
  const data: BanxaNFTOrderI<any> = {
    ...
    meta_data: {
      nft: {
        ...
        token_id: account_reference,
      }
    }
  }
  const outcome = await BANXA_PAYMENT.createNFTOrder(data, 'buy');
